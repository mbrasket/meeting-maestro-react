...
import { CleanDayColumn } from './components/CleanDayColumn';
import { TimeColumn } from './components/TimeColumn';
import { CleanDragGhost } from './components/CleanDragGhost';
...
const CleanCalendarGrid = ({
  ...
  const handleBeforeCapture = useCallback((before: BeforeCapture) => {
    console.log('Drag started with ID:', before.draggableId);
    
    const sourceType = before.draggableId.startsWith('tool-') ? 'tools' : 'calendar';
    let draggedItemType = null;
    
    if (sourceType === 'tools') {
      draggedItemType = before.draggableId.replace('tool-', '');
    } else {
      const item = items.find(item => item.id === before.draggableId);
      draggedItemType = item?.type || null;
    }
  ...
  const handleDragEnd = useCallback((result: DropResult) => {
    ...
    if (!source.droppableId.startsWith('tools-') && !destination.droppableId.startsWith('tools-')) {
      ...
      if (item && destination.droppableId.startsWith('day-')) {
        ...
        console.log('Moving item:', item.id, 'to', targetDate);
